_base_ = [
    '../_base_/datasets/irair_real_label.py', '../_base_/default_runtime.py'
]
...
num_levels = 2
in_channels = 1
embed_dims = 64  # embed_dims256
query_initial = 'one'
model = dict(
    type='SpecDetr',
    num_queries = 2000,  # num_matching_queries 900
    num_query_per_cat= 5,
    num_fix_query = 0,
    with_box_refine=True,
    as_two_stage=True,
    num_feature_levels=num_levels,
    candidate_bboxes_size=0.01,  # 1/256, #   initial candidate_bboxes after encode 0.01
    ...
    encoder=dict(
        num_layers=3,
        layer_cfg=dict(
            self_attn_cfg=dict(embed_dims=embed_dims, num_levels=num_levels, num_points=8, # local_attn_type ='fix_same_orientation',
    ...
    decoder=dict(
        ...
        layer_cfg=dict(
            self_attn_cfg=dict(embed_dims=embed_dims, num_heads=8,
                               dropout=0.0),  # 0.1 for DeformDETR
            cross_attn_cfg=dict(embed_dims=embed_dims, num_levels=num_levels, num_points=8, #local_attn_type = 'fix_same_orientation',  # fix_same_orientation
                                dropout=0.0),  # 0.1 for DeformDETR
            ffn_cfg=dict(
                embed_dims=embed_dims,
